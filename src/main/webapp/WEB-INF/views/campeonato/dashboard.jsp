...
<div class="mui-container" ng-controller="CampeonatoDashboardCrontroller as dashCtrl">
	  ...
	  <div class="mui-col-md-9 mui--text-right">
	  	  <button ng-show="usuarioLogged" href class="mui-btn mui-btn--small {{ dashCtrl.showCampeonatos == dashCtrl.TODOS ? '' : 'mui-btn--primary' }}" 
	  	  		  ng-click="dashCtrl.btnShowCampeonatos(usuarioLogged)">Meus Campeonatos</button> 
	  </div>	  	 
	  	...
	  	<div class="mui-col-md-7 mui-col-xs-7">
	  		<a ng-click="dashCtrl.enterCampeonato(c)" href>
		  		<span class="mui--text-headline">
		  			{{ c.descricao }} 
		  			<small> / {{ c.nome }} </small>
		  			<img ng-show="usuarioLogged.id == c.presidente.id" src="${pageContext.request.contextPath}/resources/static/img/star_32X32.png" />
		  		</span>
		  	</a>
		</div>
	  	<div class="mui-col-md-4 mui-col-xs-12">
  			<a ng-show="usuarioLogged.id == c.presidente.id || usuarioLogged.id == 1" href="#/campeonato/{{c.id}}?next=/">
  				<img src="/jbolao/resources/static/img/edit24X24.png" alt="editar"/> 
  			</a> &nbsp; &nbsp;&nbsp;	  	
	  		<a ng-show="usuarioLogged.id == c.presidente.id || usuarioLogged.id == 1" href="#/jogos/{{c.id}}" class="mui-btn mui-btn--small mui-btn--danger">Jogos</a>
	  	</div>
	  </div> 
