...
app.controller('InscricaoRanckingController', ['$routeParams', '$location', 'CampeonatoService', 
	...
	var _newInscricao = function(inscricao) {
		...
		inscricaoNew.acertoPlacar = inscricao.acertoPlacar;
		inscricaoNew.acertoVencedorUmResultado = inscricao.acertoVencedorUmResultado;
		inscricaoNew.acertoVencedor = inscricao.acertoVencedor;
		inscricaoNew.acertoSomenteUmResultado = inscricao.acertoSomenteUmResultado;
		inscricaoNew.errouTudo = inscricao.errouTudo;
		inscricaoNew.jogos = inscricao.jogos;
	...
	var _reordenarRanking = function() {
		...
		for(var aux in inscricoesOrder) {
			...
			self.inscricoesFilter[aux].acertoPlacar = inscricoesOrder[aux].acertoPlacar;
			self.inscricoesFilter[aux].acertoVencedorUmResultado = inscricoesOrder[aux].acertoVencedorUmResultado;
			self.inscricoesFilter[aux].acertoVencedor = inscricoesOrder[aux].acertoVencedor;
			self.inscricoesFilter[aux].acertoSomenteUmResultado = inscricoesOrder[aux].acertoSomenteUmResultado;
			self.inscricoesFilter[aux].errouTudo = inscricoesOrder[aux].errouTudo;
			self.inscricoesFilter[aux].jogos = inscricoesOrder[aux].jogos;
...
//		});
//	}
	self.executaFiltro = function(rodadas) {
		if(!(rodadas.length > 0)) {
			alert('Selecione uma rodada');
			return;
		}
		ApostaService.findByCampeonatoAndJogoRodadaInOrderByInscricao(self.campeonato, rodadas).then(function(resp) {
			self.apostasFilter = resp.data;
			return resp;
		}).then(function(apostaResp) {
			InscricaoService.findByCampeonato(self.campeonato).then(function(resp) {
				...
				for(var ii = 0; ii <= self.inscricoesFilter.length-1; ii++) {
					var pontuacao = countTotalPontosByInscricao(self.inscricoesFilter[ii]);
					self.inscricoesFilter[ii].pontos = pontuacao.pontos;
					self.inscricoesFilter[ii].acertoPlacar = pontuacao.AP;
					self.inscricoesFilter[ii].acertoVencedorUmResultado = pontuacao.AR;
	...
	self.rodadasSelecionadas = [];
	self.selecionarRodada = function(rodada) {
		if(self.rodadasSelecionadas.indexOf(rodada) > -1)
			self.rodadasSelecionadas.splice(self.rodadasSelecionadas.indexOf(rodada), 1);
		else 
			self.rodadasSelecionadas.push(rodada);
	}
